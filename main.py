...
parser.add_argument('--dataroot', default='/home/rui/IIAI/Zero-shot-teaching/data/', help='path	to dataset')
...
def save_log(path,mode):
	...
	log_dir =   os.path.join(log_path,mode, opt.dataset,time_str,time_str1)
...
def compute_acc(real_label, predicted_label):
	...
	index =	(real_label==predicted_label)
...
if opt.need_teacher:
	...
	loadpath =	os.path.join('./classifier_weight',opt.teacher_type, opt.dataset,'1104','20201104_204557','classifier.pth')
	pretrain_cls =	classifier.CLASSIFIER(data.train_feature, util.map_label(data.train_label, data.seenclasses),   
											  data.test_seen_feature,data.test_seen_label,data.seenclasses,data,data.seenclasses.size(0), 
...
for	epoch in range(opt.nepoch):
   ...
   #############################
	...
	cls_result = F.softmax(pretrain_cls.model(fake_feature),dim=1)
	...
	print('Epoch: [%d/%d],  [right_num/generated_num]:   [%d/%d]  '%(epoch, opt.nepoch, right_num,fake_feature.shape[0]) )
	print("---------------------------------------")
	...
	for	epoch_q in range(opt.epoch_q):
		...
		#############  split train/test set for quality classification
		'''
		#############  1.  input size same as trainval + test_seen
		label_q_train = label_q[:data.ntrain]
		fake_feature_train = fake_feature[:data.ntrain,:]
		label_q_test = label_q[data.ntrain:]
		fake_feature_test = fake_feature[data.ntrain:,:]

		#print('label_q_train',label_q_train)
		...
		print('train_dataset / test_dataset:  [%d/%d]  '%(int(data_num*(1-opt.val_split)),int(data_num*opt.val_split)))
		...
		for i in range(0, data.ntrain,	opt.batch_size):
			...
			pred_label = netC(input_feat)
			pred_label = torch.squeeze(pred_label)   ### tensor dimension
			#print('pred_label',pred_label)
			#print('pred_label',pred_label.shape)
			
			errC = cls_criterion(pred_label,quality_label)
		...
		print('[%d/%d]	Loss_C:	%.4f' % (epoch_q, opt.epoch_q, errC))
		print("---------------------------------------")
		#time.sleep(3)
		...
		pred_tlabel = torch.squeeze(netC(fake_feature_test)).float()
		...
		if errC<=0.0001 or cls_acc>=99.99:
	...
	print('')
	print('Time to Evaluate Generator !')
	print('')
	...
	if opt.gzsl:
		...
		allclasses = torch.cat((data.seenclasses,data.unseenclasses),0)

		syn_feat, syn_label = generate_syn_feature(netG, allclasses, data.attribute, opt.syn_num)  ### generate feature
		
		Syn_feature = torch.FloatTensor(syn_feat.shape[0], opt.resSize)	
		Syn_feature.copy_(syn_feat)
		syn_feature = Variable(Syn_feature, requires_grad=False).cuda()
		
		pred_syn_label =torch.squeeze(netC(syn_feature)).float()
		#print('pred_syn_label',pred_syn_label)
		syn_candidate_idx =( pred_syn_label>=0.5) 
		syn_ratio = torch.sum(syn_candidate_idx).float() / torch.tensor(syn_feature.shape[0]).float()

		print('Ratio of candidate syn feature: %.4f, [%d/%d] '%( syn_ratio,torch.sum(syn_candidate_idx),syn_feature.shape[0]))


		
		if torch.sum(syn_candidate_idx) ==0:
			print('Fail to continue!')
			
		else:


			print('Preparing the syn feature for training classifier!')
			print('---------------------------------------------------')
			print('')
			idx_candidate =torch.squeeze((syn_candidate_idx==1).nonzero())  ###  find index of correct labeled syn feature		
	
			train_X = syn_feat[idx_candidate]
			train_Y = syn_label[idx_candidate]
			nclass = opt.nclass_all
			#print('train_X',train_X.shape)
			
			
			
			###########   use pretrain allclasses classifier to check the quality of  syn_feature_candidate   ##################
			loadpath =	os.path.join('./classifier_weight','all_classes', opt.dataset,'1107','20201107_152000','classifier.pth')
			pretrain_allclasses = torch.load(loadpath)
			
			train_X = train_X.cuda()
			train_Y = train_Y.cuda()
			cls_result = F.softmax(pretrain_allclasses.model(train_X),dim=1)
			#print('cls_result',cls_result)
			#print('cls_result',cls_result.shape)
	...
	# Zero-shot learning
		...
		syn_feat, syn_label = generate_syn_feature(netG, data.unseenclasses, data.attribute, opt.syn_num)  ### generate feature
		...
		if torch.sum(syn_candidate_idx) ==0:
			...
			cls = classifier2.CLASSIFIER(train_X, util.map_label(train_Y, data.unseenclasses), 
										 data, data.unseenclasses.size(0), opt.cuda, 
										 opt.classifier_lr, 0.5, 25, opt.batch_size, False)
